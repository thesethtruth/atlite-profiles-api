...
from core.cutout_processing import (
    get_wind_profile, 
    get_solar_profile, 
    print_available_turbine_list,
    get_turbine_data
)
...
class ProfileConfig:
    ...
    output_dir: Path
    base_path: Path = Path("/mnt/c/Users/BERC2/Data")
    cutouts: List[str] = field(default_factory=lambda: [
        "europe-1987-era5.nc", 
        "europe-2012-era5.nc", 
        "europe-2021-era5.nc", 
        "europe-2022-era5.nc", 
    ...
    location: Dict[str, float] = field(default_factory=lambda: {"lat": 52.705, "lon": 4.770})
    run_single_iteration: bool = False
...
class ProfileGenerator:
    ...
    def generate_wind_profiles(self) -> Dict[str, pd.Series]:
        ...
        for cutout in self.config.cutouts:
            ...
            if filepath.exists():
                print(f"Wind profile for {cutout_year} already exists at {filepath}, loading existing file")
                wind_profile = pd.read_csv(filepath, index_col=0, parse_dates=True).squeeze()
                profiles[profile_key] = wind_profile
                print(f"Full load hours: {wind_profile.sum()}")
            else:
                print(f"Generating wind profile for {cutout_year} using {self.wind_config.turbine_model}")
            ...
            if self.config.run_single_iteration:
                break
    ...
    def generate_solar_profiles(self) -> Dict[str, pd.Series]:
        ...
        for cutout in self.config.cutouts:
            ...
            for slope, azimuth in zip(self.solar_config.slopes, self.solar_config.azimuths):
                ...
                # Check if file already exists
                if filepath.exists():
                    print(f"Solar profile for {cutout_year} with slope={slope}, azimuth={azimuth} already exists at {filepath}, loading existing file")
                    solar_profile = pd.read_csv(filepath, index_col=0, parse_dates=True).squeeze()
                    profiles[profile_key] = solar_profile
                    print(f"Full load hours: {solar_profile.sum()}")
                else:
                    print(f"Generating solar profile for {cutout_year} with slope={slope}, azimuth={azimuth}")
    ...
    def visualize_solar_profiles_monthly(self, color_key: Literal["slope", "azimuth", "year"] = "slope"):
    ...
    @staticmethod
    def list_available_turbines():
        """List all available turbine models."""
        print_available_turbine_list()
